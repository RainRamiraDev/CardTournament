...
{
    public class UserCreationDtoValidation : AbstractValidator<UserCreationDto>
    {
        public UserCreationDtoValidation()
        {
            RuleFor(user => user.Id_Country)
            .GreaterThan(0).WithMessage("El Id_Country debe ser un valor positivo.");

            RuleFor(user => user.Id_Rol)
